...
function viewDashboard(seg, q) {
  ...
  ${tab === 'overview' ? `
    ...
    <div class="panel"><h3 style="margin-bottom:.7rem">Latest orders</h3>${myO.length ? sellerOrderRows(myO.slice(0, 5)) : '<table class="table"><tr><th>Order</th><th>Items</th><th>Buyer pays</th><th>You get</th></tr><tr class="ghost"><td>o_····</td><td>1× Your first sale</td><td>$ ··.··</td><td><b>total − 10%</b></td></tr></table><p style="color:var(--ink3);font-size:.8rem;margin-top:.5rem">Orders land here — share your storefront link to get the first one.</p>'}</div>` : ''}
  ${tab === 'products' ? `
    ...
    <div class="tbl-wrap panel"><table class="table"><tr><th>Item</th><th>Price</th><th>Stock</th><th>Sold</th><th>Views</th><th></th></tr>
      ${myP.map(p => `<tr><td style="max-width:280px"><b style="font-size:.85rem">${esc(p.title)}</b><br><span class="badge badge-${p.cond}">${condName(p.cond)}</span></td>
        ...
        <td style="white-space:nowrap"><button class="btn btn-outline btn-sm" onclick="openProductForm('${p.id}')">Edit</button> <button class="btn btn-ghost btn-sm" onclick="delProduct('${p.id}')">🗑</button></td></tr>`).join('') || '<tr class="ghost"><td style="max-width:280px"><b style="font-size:.85rem">Your first listing</b><br><span class="badge badge-new">New</span></td><td>$ ···</td><td>·</td><td>0</td><td>0</td><td style="font-size:.78rem">appears here — hit + New listing</td></tr>'}</table></div>` : ''}
  ${tab === 'codes' ? `
    ...
    <div class="tbl-wrap panel"><table class="table"><tr><th>Code</th><th>Discount</th><th>Min order</th><th>Used</th><th>Status</th><th></th></tr>
      ${myC.map(c => { const dead = !c.active || (c.expires && c.expires < Date.now()) || (c.max != null && c.uses >= c.max);
        ...
        <td><button class="btn btn-ghost btn-sm" onclick="toggleCode('${c.id}')">${c.active ? 'Disable' : 'Enable'}</button></td></tr>`; }).join('') || '<tr class="ghost"><td><b>SUMMER10</b></td><td>10%</td><td>—</td><td>0</td><td><span class="badge badge-verified">live</span></td><td style="font-size:.78rem">your first private promo code appears here</td></tr>'}</table></div>` : ''}
  ${tab === 'orders' ? `<div class="panel tbl-wrap">${myO.length ? sellerOrderRows(myO) : '<table class="table"><tr><th>Order</th><th>Items</th><th>Buyer pays</th><th>You get</th><th>Status</th><th></th></tr><tr class="ghost"><td>o_····</td><td>1× Your first sale</td><td>$ ··.··</td><td><b>total − 10%</b></td><td><span class="badge badge-gray">paid</span></td><td></td></tr></table>'}</div>` : ''}
